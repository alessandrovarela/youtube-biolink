...
| 2026-07-27 | 2.0     | **Produto entregue e em produção.** Epics 1–6 concluídos: MVP funcional (6 pilares — auth, perfil, links, página pública, temas, analytics) + hardening (Epic 6 — RLS nas 3 tabelas com view `link_click_daily` blindada, rate limiting Supabase-native, proxy de borda com auth guard + CSP). Deploy verificado em produção (PRE-M7); smoke test manual e validação visual dos 3 temas executados pelo owner com sucesso; tags `v0.1.0` (MVP) e `v0.2.0` (Epic 6). Encerra a Story 5.6 AC5 (promoção do Change Log). **Aprovado pelo owner (Alessandro Varela).** | Morgan (PM) |
